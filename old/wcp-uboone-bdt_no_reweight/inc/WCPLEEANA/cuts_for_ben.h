 ...
 // nueCC 3 variables: n_trakcs, n_showers, gap_n_bad, FC/PC x3 = 6 channels; 4 additional channels 
  ...
  // NC delta (1g) channels:
            ...
            if (flag_FC && flag_ncdelta_sel && flag_0p && (eval.truth_isCC==0 && pfeval.truth_NCDelta==1 && flag_truth_inside)){
                ...
                return ((nTrueP==0 && ch_name=="nc_delta_0p_01_true_Np_nc_delta_overlay") || (nTrueP>0 && ch_name=="nc_delta_0p_01_true_Np_nc_delta_overlay"));
            }
            return false;
    ...
    // numuCC channels (with NC delta and NC pi0 events removed):
            ...
            if (flag_FC && flag_numuCC && (!flag_ncpio_sel) && (!flag_ncdelta_sel) && (!flag_0p) && (eval.truth_isCC==0 && pfeval.truth_NCDelta==1 && flag_truth_inside)){
                ...
                return ((nTrueP==0 && ch_name=="numuCC_Np_true_0p_nc_delta_overlay") || (nTrueP>0 && ch_name=="numuCC_Np_true_Np_nc_delta_overlay"));
            }
            return false;
            ...
            if (flag_FC && flag_numuCC && (!flag_ncpio_sel) && (!flag_ncdelta_sel) && flag_0p && (eval.truth_isCC==0 && pfeval.truth_NCDelta==1 && flag_truth_inside)){
                ...
                return ((nTrueP==0 && ch_name=="numuCC_0p_true_0p_nc_delta_overlay") || (nTrueP>0 && ch_name=="numuCC_0p_true_Np_nc_delta_overlay"));
            }
            return false;
...
bool LEEana::get_rw_cut_pass(TString cut, EvalInfo& eval, PFevalInfo& pfeval, TaggerInfo& tagger, KineInfo& kine){
  if(cut == "NCPi0"){
    if (eval.truth_isCC==0 && pfeval.truth_NprimPio>0 && is_FC(eval)) return true;
    return false;
  }else if(cut == "NCPi0_Np"){
    if (eval.truth_isCC==0 && pfeval.truth_NprimPio>0 && is_FC(eval) && !(is_true_0p(pfeval)) ) return true;
    return false;
  }else if(cut == "NCPi0_0p"){
    if (eval.truth_isCC==0 && pfeval.truth_NprimPio>0 && is_FC(eval) && is_true_0p(pfeval) ) return true;
    return false;
  }else if(cut == "NCDeltaNp_scale"){
    if(is_FC(eval) && is_NCdelta_sel(tagger, pfeval) && eval.truth_isCC==0 && pfeval.truth_NCDelta==1 && !(is_0p(tagger, kine, pfeval))) return true;
    return false;
