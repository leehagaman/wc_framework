...
#include "init.txt"
  ...
  for (Int_t i=0;i!=T_eval->GetEntries();i++){
    ...
    if (pfeval.reco_Ntrack > 0) { 
      std::cout << "lhagaman debug, reco_Ntrack = " << pfeval.reco_Ntrack << "\n";
    }
    if (pfeval.truth_Ntrack > 0) { 
      std::cout << "lhagaman debug, truth_Ntrack = " << pfeval.truth_Ntrack << "\n";
    }
