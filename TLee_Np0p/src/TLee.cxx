...
double TLee::FCN_Np_0p(const double *par)
{
	...
	*/
	...
	//cout << "lhagaman debug, matrix_cov_syst(6,6): " << matrix_cov_syst(6,6) << "\n";
	...
	if (use_custom_joint_chi2) {
		...
		if (four_bin_test) { // wc 1gNp and wc 1gNp overflow (empty)
			ind_tar_start = 0;
			ind_tar_end = 0 + 2*4-1; // two bins, four channels, leaving out overflow
			ind_constr_start = 2*4-1; // using that overflow to constrain (effectively no constraint)
			ind_constr_end = 2*4;
		}

		if (eight_bin_test) { // wc 1gNp and wc 1gNp overflow (empty)
			ind_tar_start = 0;
			ind_tar_end = 0 + 4*4-1; 
			ind_constr_start = 4*4-1;
...
void TLee::Set_Variations(int num_toy)
{
	...
	// this seed setting was making it so different threads get the same result, not desirable
	//rand->SetSeed(42);
...
void TLee::Set_Collapse()
{
	...
	//cout << "lhagaman debug, matrix_pred_oldworld (up to all):\n";
	//for(int ibin=0; ibin<bins_oldworld; ibin++) cout << "  " << matrix_pred_oldworld(0, ibin) << "\n";
	//cout << "lhagaman debug, done printing matrix_pred_oldworld\n";
	...
	/*
	   cout << "getting matrix_pred_newworld from matrix_pred_oldworld * matrix_transform_Lee:\n"; 
	   cout << "    shapes: (" << matrix_pred_oldworld.GetNrows() << "," << matrix_pred_oldworld.GetNcols() << ") * (" << matrix_transform_Lee.GetNrows() << "," << matrix_transform_Lee.GetNcols() << ") = (" << matrix_pred_newworld.GetNrows() << "," << matrix_pred_newworld.GetNcols() << ")\n"; 
	   cout << "    mins: " << matrix_pred_oldworld.Min() << "," << matrix_transform_Lee.Min() << "," << matrix_pred_newworld.Min() << "\n"; 
	   */
	...
	//cout << "lhagaman 4, matrix_absolute_cov_oldworld.Min(): " << matrix_absolute_cov_oldworld.Min() << "\n";


	// start additions for statistical correlations, lhagaman 2023_07_20
	...
	//cout << "lhagaman 5, matrix_absolute_cov_oldworld.Min(): " << matrix_absolute_cov_oldworld.Min() << "\n";

	//cout << "map_input_spectrum_oldworld_bin.Min(): " << map_input_spectrum_oldworld_bin.Min() << "\n";

	//cout << "matrix_input_cov_flux_Xs.Min(): " << matrix_input_cov_flux_Xs.Min() << "\n";
	//cout << "matrix_input_cov_flux.Min(): " << matrix_input_cov_flux.Min() << "\n";
	//cout << "matrix_input_cov_Xs.Min(): " << matrix_input_cov_Xs.Min() << "\n";
	...
	//cout << "lhagaman 5.2, matrix_absolute_cov_oldworld.Min(): " << matrix_absolute_cov_oldworld.Min() << "\n";
	if( flag_syst_additional ) matrix_absolute_cov_oldworld += matrix_input_cov_additional;

	//cout << "lhagaman 6, matrix_absolute_cov_oldworld.Min(): " << matrix_absolute_cov_oldworld.Min() << "\n";
	...
	//cout << "    mins: " << matrix_absolute_cov_oldworld.Min() << "," << matrix_transform_Lee.Min() << "," << matrix_absolute_cov_newworld.Min() << "\n"; 
...
void TLee::Set_Spectra_MatrixCov()
{
	...
	//cout << "lhagaman flags, expect 1 0 0: " << flag_syst_flux_Xs << ", " << flag_syst_reweight << ", " << flag_syst_reweight_cor << "\n"; 
  ...
  for(int ibin=0; ibin<bins_oldworld; ibin++) {
    for(int jbin=0; jbin<bins_oldworld; jbin++) {
      ...
      if (val_i < 0) cout << "val_i negative! ibin = " << ibin << ", val_i = " << val_i << "\n";
  ...
  if(flag_syst_mc_data_stat_cor){
        ...
        TFile *roofile_data = new TFile("./data_inputs/hist_rootfiles/run_data_stat.root", "read"); // lhagaman changed 2023_07_02
        TFile *roofile_pred = new TFile("./data_inputs/hist_rootfiles/run_pred_stat.root", "read"); // lhagaman changed 2023_07_02
