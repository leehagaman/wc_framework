...
void cal_CL_Mixed_Asimov_Data_Wilks()
{
  ...
  for(auto it_Np=map_vec_toys_dchi2.begin(); it_Np!=map_vec_toys_dchi2.end(); it_Np++) {
	...
	if( dchi2_toy<map_data_dchi2[grid_Np][grid_0p] ) line_CL++;
      ...
      h2d_space->SetBinContent(grid_Np, grid_0p,  1 - TMath::Prob(map_data_dchi2[grid_Np][grid_0p],2));
  ...
  h2d_space->Draw("colz");
  h2d_space->SaveAs("file_map_CL_mixed_Asimov_data_Wilks.root");
