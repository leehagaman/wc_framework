...
void plot_nu_energy_resolution(){
  ...
  for(int i=0; i<10; i++) {
    ...
    T_eval->Project(Form("hfc%d",i), "1e-3*reco_nuEnergy", Form("weight_cv*weight_spline*(abs(truth_nuPdg)==14 && truth_isCC==1 && truth_vtxInside==1 && T_BDTvars.numu_cc_flag>=0 && T_BDTvars.numu_score>0.9 && match_isFC==1 && truth_nuEnergy>%f && truth_nuEnergy<%f)", etrue1, etrue2) );
    T_eval->Project(Form("hpc%d",i), "1e-3*reco_nuEnergy", Form("weight_cv*weight_spline*(abs(truth_nuPdg)==14 && truth_isCC==1 && truth_vtxInside==1 && T_BDTvars.numu_cc_flag>=0 && T_BDTvars.numu_score>0.9 && match_isFC==0 && truth_nuEnergy>%f && truth_nuEnergy<%f)", etrue1, etrue2) );

    // hfc.at(i)->SetTitle(Form("True energy bin: %.3f - %.3f", 1e-3*etrue1, 1e-3*etrue2));
    hfc.at(i)->SetTitle("");
    hfc.at(i)->GetXaxis()->SetTitle("Reco Neutrino Energy (GeV)");
  ...
  // lg->AddEntry(l1, "Range of E^{true}_{#nu}","l")->SetTextColor(4);
  lg->AddEntry(b1, "E^{true}_{#nu} Bins","f")->SetTextColor(4);
  lg->Draw();
