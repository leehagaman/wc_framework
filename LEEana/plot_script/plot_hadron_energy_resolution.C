...
void plot_hadron_energy_resolution(){
  ...
  double xbins1[] = {0.03, 0.15, 0.275, 0.411, 0.502, 0.614, 0.75, 1.12, 2.5}; // GeV
  ...
  for(int i=0; i<8; i++) {
    ...
    hfc.at(i)->SetLineWidth(2);
    hfc.at(i)->GetXaxis()->SetTitleSize(0.07);
    hpc.at(i) = new TH1F(Form("hpc%d",i),Form("hpc%d",i),25,0,2.5);
    hpc.at(i)->SetLineColor(2);
    hpc.at(i)->SetLineWidth(2);
    hpc.at(i)->GetXaxis()->SetTitleSize(0.07);

    T_eval->Project(Form("hfc%d",i), "1e-3*reco_hadEnergy", Form("weight_cv*weight_spline*(abs(truth_nuPdg)==14 && truth_isCC==1 && truth_vtxInside==1 && T_BDTvars.numu_cc_flag>=0 && T_BDTvars.numu_score>0.9 && match_isFC==1 && truth_hadEnergy>%f && truth_hadEnergy<%f && T_PFeval.reco_muonMomentum[3]>0)", etrue1, etrue2) );
    T_eval->Project(Form("hpc%d",i), "1e-3*reco_hadEnergy", Form("weight_cv*weight_spline*(abs(truth_nuPdg)==14 && truth_isCC==1 && truth_vtxInside==1 && T_BDTvars.numu_cc_flag>=0 && T_BDTvars.numu_score>0.9 && match_isFC==0 && truth_hadEnergy>%f && truth_hadEnergy<%f && T_PFeval.reco_muonMomentum[3]>0)", etrue1, etrue2) );

    // hfc.at(i)->SetTitle(Form("True energy bin: %.3f - %.3f", 1e-3*etrue1, 1e-3*etrue2));
    hfc.at(i)->SetTitle("");
    hfc.at(i)->GetXaxis()->SetTitle("Reco Hadron Energy (GeV)");
  ...
  lg->AddEntry(b1, "#nu (= E^{true}_{#nu} - E^{true}_{#mu}) Bins","f")->SetTextColor(4);
  lg->Draw();
