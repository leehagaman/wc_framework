void convert_wiener_7ch_fake(){
  ...
  for (Int_t i=0;i!=26;i++){
    ...
    // nueCC FC, data - bkg (no ext to subtract)
    hmeas->SetBinContent(i+1, hdata_obsch_1->GetBinContent(i+1) - histo_2->GetBinContent(i+1));
    // nueCC PC
    hmeas->SetBinContent(i+1 + 26, hdata_obsch_2->GetBinContent(i+1) - histo_4->GetBinContent(i+1));    
    // numuCC FC
    hmeas->SetBinContent(i+1 + 2*26, hdata_obsch_3->GetBinContent(i+1) - histo_6->GetBinContent(i+1));
    // numuCC PC
    hmeas->SetBinContent(i+1 + 3*26, hdata_obsch_4->GetBinContent(i+1) - histo_8->GetBinContent(i+1));
  ...
  for (Int_t i=0; i!=11; i++){
    ...
    // numuCC Pi0 FC
    hmeas->SetBinContent(i+1 + 4*26, hdata_obsch_5->GetBinContent(i+1) - histo_10->GetBinContent(i+1));
    // numuCC Pi0 PC
    hmeas->SetBinContent(i+1 + 4*26 + 11, hdata_obsch_6->GetBinContent(i+1) - histo_12->GetBinContent(i+1));
    // NC Pi0 PC
    hmeas->SetBinContent(i+1 + 4*26 + 2*11, hdata_obsch_7->GetBinContent(i+1) - histo_14->GetBinContent(i+1));
