...
struct PFevalInfo{
  ...
  // this to bracket copied from Ben
  // PF particle
...
void LEEana::clear_pfeval_info(PFevalInfo& tagger_info){
  ...
  for (Int_t i=0;i!=4;i++){
    ...
    tagger_info.truth_startMomentum[i] = 0; // lhagaman added
  }
}
...
void LEEana::set_tree_address(TTree *tree0, PFevalInfo& tagger_info, int flag){
  ...
  if (tree0->GetBranch("reco_showerMomentum")){
    ...
    if (flag==1){
      ...
      tree0->SetBranchAddress("truth_startMomentum",&tagger_info.truth_startMomentum[0]); // lhagaman added
...
void LEEana::put_tree_address(TTree *tree0, PFevalInfo& tagger_info, int flag){
  ...
  if (tagger_info.flag_showerMomentum){
    ...
    if (flag==1){
      ...
      tree0->Branch("truth_startMomentum",&tagger_info.truth_startMomentum[0],"truth_startMomentum[4]/F"); // lhagaman added
  ...
  // this to end is copied from Ben
